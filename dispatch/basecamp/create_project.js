 ...
 */
async function awaitProjectConstruction(
  basecampToken,
  templateId,
  constructionId,
) {
...
async function createProject(
  basecampToken,
  { name, description, template, subscribers = [] },
) {
  let newProject;
  let projectId;
  ...
  if (template) {
    ...
    const projectConstruction = await axios
      .post(
    ...
    projectId = await awaitProjectConstruction(
      basecampToken,
      template,
      constructionId,
    );
    // Fetch the newly created project details
    newProject = await axios
      .get(
