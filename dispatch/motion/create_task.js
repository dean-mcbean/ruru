...
async function createTask({ title, projectId, content, labels, assigneeId }) {
  return axios.post('https://api.usemotion.com/v1/tasks', {
    name: title,
    projectId: projectId,
    ...
    response.taskUrl = `https://app.usemotion.com/web/pm/workspaces/${response.data.workspace.id}?task=${response.data.id}`;
    return response;
  }).catch(error => {
