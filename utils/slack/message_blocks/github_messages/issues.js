...
function generateDescription(data) {
  ...
  if (data.issue.additions)
    description.push(
      `${data.issue.additions} Addition${sIf(data.issue.additions)}`,
    );
  if (data.issue.deletions)
    description.push(
      `${data.issue.deletions} Deletion${sIf(data.issue.deletions)}`,
    );
  if (data.issue.changed_files)
    description.push(
      `${data.issue.changed_files} Changed File${sIf(data.issue.changed_files)}`,
    );
...
async function generateMessageContentForIssue(data) {
  ...
  const persistent_issue_status = await usePersistentItem(
    "issues",
    "issue_status",
  ...
  if (issue_status && issue_status.description)
    description = issue_status.description;
