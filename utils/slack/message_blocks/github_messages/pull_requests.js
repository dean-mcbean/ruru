...
const uploadFileToSlack = require("../../../../dispatch/slack/upload_file");
const {
  BlockMessageBuilder,
  createButtonBlock,
...
} = require("../../../../utils/mongodb/get_user");
const {
  usePersistentItem,
} = require("../../../../utils/mongodb/persistent_item");
const {
  bindAction,
} = require("../../../../utils/slack/message_blocks/action_handler");
const {
  addReviewerToPullRequest,
} = require("../../../../utils/slack/message_blocks/actions/pull_requests");
const axios = require("axios");
const fs = require("fs");
const { GITHUB, RURU_PUBLIC, URLS } = require("../../../../constants");
...
function generateDescription(data) {
  ...
  if (data.pull_request.additions)
    description.push(
      `${data.pull_request.additions} Addition${sIf(data.pull_request.additions)}`,
    );
  if (data.pull_request.deletions)
    description.push(
      `${data.pull_request.deletions} Deletion${sIf(data.pull_request.deletions)}`,
    );
  if (data.pull_request.changed_files)
    description.push(
      `${data.pull_request.changed_files} Changed File${sIf(data.pull_request.changed_files)}`,
    );
...
async function getImageBlobsFromDescription(description) {
  ...
  const htmlImgUrls = description.match(
    /<img[^>]*src=["'](https?:\/\/[^"']+)["'][^>]*>/g,
  );
  if (htmlImgUrls) {
    imageUrls.push(
...
async function uploadImageUrlsToServer(description) {
  ...
  return imageNames.map((x) => `${RURU_PUBLIC.BASE_URL}/images/${x}`);
}
...
async function uploadImageUrlsToSlack(description) {
  ...
  for (const image of images) {
    const imageUrl = await uploadFileToSlack(
      image,
      image.split("/").pop(),
      process.env.DEV_CHAT_CHANNELID,
    );
    if (imageUrl) {
...
function replaceImagesWithWords(description) {
  ...
  let index = 1;
  return description
    .replace(/!\[image\]\((https?:\/\/[^\s]+)\)/g, (match, url) => {
      return `<${url}|🖼️ (image)>`;
    })
    .replace(
      /<img[^>]*src=["'](https?:\/\/[^"']+)["'][^>]*>/g,
...
async function generateMessageContentForPullRequest(data) {
  ...
  const persistent_pr_status = await usePersistentItem(
    "pull_requests",
    "pr_status",
    pr_url[pr_url.length - 1],
  );
  ...
  const readable_action = data.action
    .replace(/_/g, " ")
  ...
  if (data.pull_request.merged_at) {
    ...
    ["closed", "opened", "reopened", "review_requested", "edited"].includes(
      data.action,
    )
  ) {
  ...
  if (
    ...
    !(data.review && data.action == "submitted")
  ) {
    const pr_url = data.pull_request.html_url.split(".");
    button = createButtonBlock({
      ...
      action_id: bindAction(
        "pull_request.add_reviewer",
        addReviewerToPullRequest,
      ),
    });
  }
...
async function fetchRecentlyMergedPRs(repo) {
  ...
  const url = `${URLS.githubApi}/repos/${GITHUB.OWNER}/${repo}/pulls?state=closed&base=main&per_page=100`;
  const response = await axios.get(url, {
    headers: {
...
module.exports = {
  generateMessageContentForPullRequest,
