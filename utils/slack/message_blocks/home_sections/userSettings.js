const {
  notificationOptions,
} = require("../../storage_utils/get_notifications");
const {
  getUsergroupRepos,
} = require("../../storage_utils/get_usergroup_repos");
const { bindAction } = require("../../webhook_handlers/slack/action_handler");
const {
  backToDefault,
} = require("../../webhook_handlers/slack/actions/back_to_default");
const {
  setGithubUsername,
  setNotifications,
} = require("../../webhook_handlers/slack/actions/user_settings");
...
async function userSettings({ userConfigValue, bmb }) {
  ...
  if (userConfigValue.notifications) {
    initial_options = notificationOptions.filter(
      (option) => userConfigValue.notifications[option.value],
    );
  }
  bmb.addCheckboxList({
  ...
  const usergroup = devUsers.includes(userConfigValue.id)
    ? "Developer"
    : analystUsers.includes(userConfigValue.id)
      ? "Analyst"
      : "None";
