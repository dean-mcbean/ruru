const { usePersistentItem } = require("../../../mongodb/persistent_item");
const { bindAction } = require("../action_handler");
const { openUserSettings } = require("../actions/user_settings");
const { openPatchNotes } = require("../actions/patch_notes");
const { capitalize, createButtonBlock } = require("../message_blocks");
const { GITHUB, SLACK_HOME_TRACKED_REPOS } = require("../../../../constants");

const projectsToTrack = SLACK_HOME_TRACKED_REPOS.length
  ? SLACK_HOME_TRACKED_REPOS
  : ["your-main-repo"];
...
async function defaultHomeForDev({ user_name, bmb, sortedPRs, sortedDeploys }) {
  ...
  const sortedPipelines = Object.entries(pipelineStatusValue || {}).sort(
  ...
  bmb
    ...
    .addSection({
      text: `*Pipeline Status*${
        pipelineStatusValue
          ? sortedPipelines.reduce(
              (acc, [key, value]) =>
                key !== "_id"
                  ? `${acc}\n ● *${key}:* ${value.status} (${new Date(value.lastUpdated).toLocaleString()})`
                  : acc,
              "",
            )
    ...
    if (sortedDeploys[project] && sortedPRs[project]) {
      ...
      for (const stage of stagesToTrack) {
        ...
        if (sortedDeploys[project][stage]) {
          ...
          lastDeploy = convertLastDatetime(
            `${sortedDeploys[project][stage].created_at}`,
          );
        }
        ...
        const hasprs =
          sortedPRs[project] && sortedPRs[project][stage].length > 0;
        bmb.addSection({
...
`,
          accessory:
            stage !== "dev"
              ? createButtonBlock({
                  text: `:ruru-deploy:`,
                  url: `https://github.com/${GITHUB.OWNER}/${project}/actions/workflows/DeployTo${capitalize(stage)}.yml`,
    ...
    bmb.addPadding({ padding: 2 }).addDivider();
  }
