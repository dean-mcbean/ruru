...
const addReviewerToPullRequest = async (action, data) => {
  ...
  const pr_status = await usePersistentItem(
    "pull_requests",
    "pr_status",
    pull_request_id,
  );
  ...
  pr_status_value.message_blocks[0].text.text =
    pr_status_value.message_blocks[0].text.text.replace(
      /\|:([^:]+):/g,
      `|:eyes:`,
    );

  // Update the recent action summary
  const context_index = pr_status_value.message_blocks.findIndex(
    (b) => b.type == "context",
  );
  const split_context =
    pr_status_value.message_blocks[context_index].elements[0].text.split("|");
  pr_status_value.action_summary = `Under Review by ${data.user.name}`;
  pr_status_value.message_blocks[context_index].elements[0].text =
    split_context[0] +
    "|" +
    split_context[1] +
    `|  Under Review by ${data.user.name}  |` +
    split_context[2];
