...
const getHubspotDeals = async (req, res) => {
  ...
  const mappedResult = result.flatMap((deal) => {
    const saasIndex = HUBSPOT.SAAS.findIndex(
      (stage) => stage.id == deal.properties.dealstage,
    );
    const consultingIndex = HUBSPOT.CONSULTING_GRANTS.findIndex(
      (stage) => stage.id == deal.properties.dealstage,
    );
    ...
    if (
      (!isSaas && !isConsulting) ||
      (isSaas && saasStageInvalid) ||
      (isConsulting && consultingStageInvalid)
    ) {
      return [];
    }
    return [
      {
        ...deal,
        stageLabel: isSaas
          ? HUBSPOT.SAAS[saasIndex].label
          : isConsulting
            ? HUBSPOT.CONSULTING_GRANTS[consultingIndex].label
            : null,
