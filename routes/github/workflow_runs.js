...
const { downloadArtifact } = require("../../fetch/github/artifacts");
const sendMessage = require("../../dispatch/slack/send_message");
const { BlockMessageBuilder, createButtonBlock } = require("../../utils/slack/message_blocks/message_blocks");
const { usePersistentItem } = require("../../utils/mongodb/persistent_item");
const fs = require('fs');
const { RURU_PUBLIC, PLAYWRIGHT } = require('../../constants');
...
const handlePlaywrightTestEvent = async (data, triggering_workflow, workflow_id) => {
  ...
  await downloadArtifact(workflow_id, 'playwright-report', PLAYWRIGHT.ARTIFACT_REPO);
  ...
  if (shouldSend) {
    ...
    blocks.addSection({text: `:mild_panic: *Playwright Tests Failed for workflow ${triggering_workflow}*`,
      accessory: createButtonBlock({
        ...
        url: `${RURU_PUBLIC.BASE_URL}/hosted/playwright-report/`
    })});
