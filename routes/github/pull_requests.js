...
} = require("../../utils/slack/message_blocks/github_messages/pull_requests");
...
async function notifyUserOnPRUpdate(slack_user, data) {
  if (!slack_user.notifications || !slack_user.notifications.pull_requests)
    return;
...
const handlePullRequestEvent = async (data) => {
  ...
  const slack_user = await getUserByGithubUsername(
  ...
  if (data.pull_request.merged && data.pull_request.base.ref === "main") {
    const devStage = await usePersistentItem(
      "projects",
