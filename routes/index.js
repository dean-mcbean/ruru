...
const { HUBSPOT_WEBHOOKS } = require('../constants');
...
router.post('/hubspot/', async (req, res, next) => {
  ...
  try { 
    for (const deal of req.body) {
      if (
        HUBSPOT_WEBHOOKS.DEALSTAGE_RUNN_TRIGGER_ID &&
        deal.propertyName == 'dealstage' &&
        deal.propertyValue == HUBSPOT_WEBHOOKS.DEALSTAGE_RUNN_TRIGGER_ID
      ) {
