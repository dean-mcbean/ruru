...
export const requireAuth = async (to, from, next) => {
  if (process.env.VUE_APP_IN_DEVELOPMENT === "true") {
    console.log(
      "[authGuard] Development mode: skipping auth check & setting dummy auth state",
    );
    next();
    return;
  }
  ...
  if (!authStore.isLoggedIn) {
    ...
    try {
      ...
      if (isAuthenticated) {
        ...
        console.log(
          "[authGuard] Not authenticated after check, redirecting to login",
        );
        next({ name: "login", query: { redirect: to.fullPath } });
      }
    } catch (error) {
      console.log(
        "[authGuard] Auth check failed, logging out and redirecting",
        error,
      );
      authStore.logout();
      next({ name: "login", query: { redirect: to.fullPath } });
