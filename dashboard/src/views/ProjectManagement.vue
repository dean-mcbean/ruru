<template>
  <div class="dashboard-view">
    ...
    <div class="project-table">
      <div>
        <div class="table-header">
          ...
          <div
            class="pill"
            v-if="basecampLoaded === 'error'"
            @click="loadBasecampProjects"
          >
            Error <VIcon icon="mdi-refresh" />
          </div>
        </div>
        <DataColumn
          title="Basecamp"
          ...
          <template #default="{ item }">
            <div class="project-item-column">
              <div class="project-item-header">
                <a target="_blank" :href="item.app_url"
                  ><strong
                    ><SearchHighlight
                      :text="item.name"
                      :query="searchQuery" /></strong
                ></a>
                <VMenu open-on-hover>
                  ...
                  <VList>
                    <VListItem
                      @click="navigator.clipboard.writeText(item.app_url)"
              ...
              <div class="project-item-content">
                ...
                <span
                  >End Date:
                  {{
                    item.end_date ? formatDate(new Date(item.end_date)) : "N/A"
                  }}</span
                >
              </div>
            </div>
      ...
      <div>
        <div class="table-header">
          ...
          <div
            class="pill"
            v-if="runnLoaded === 'error'"
            @click="loadRunnProjects"
          >
            Error <VIcon icon="mdi-refresh" />
          </div>
        </div>
        ...
        >
          <template #default="{ item }">
            <div class="project-item-column">
              <div class="project-item-header">
                <a target="_blank" :href="`${runnAppBase}/projects/${item.id}`"
                  ><strong
                    ><SearchHighlight
                      :text="item.name"
                      :query="searchQuery" /></strong
              ...
              <div class="project-item-content">
                <div>
                  <SearchHighlight
                    :text="
                      runnClients.find((c) => c.id === item.clientId)?.name ||
                      'N/A'
                    "
                    :query="searchQuery"
                  />
                ...
                <div>
                  <template
                    v-if="item.budget !== undefined && item.budget !== null"
                  >
                    {{ "$" + item.budget }}
                  </template>
                  <template v-else>
      ...
      <div>
        <div class="table-header">
          ...
          <div
            class="pill"
            v-if="hubspotLoaded === 'error'"
            @click="loadHubspotProjects"
          >
            Error <VIcon icon="mdi-refresh" />
          </div>
        </div>
            ...
            <div class="project-item-column">
              <div class="project-item-header">
                ...
                <VMenu open-on-hover>
                  ...
                  <VList>
                    <VListItem
                      @click="copyHubspotToRunn(item)"
                      style="cursor: pointer"
                    >
                      <VListItemTitle>Duplicate in Runn</VListItemTitle>
                    </VListItem>
                    <VListItem
                      @click="navigator.clipboard.writeText(item.url)"
                      style="cursor: pointer"
                    >
                      <VListItemTitle>Copy Link</VListItemTitle>
                    </VListItem>
              ...
              <div class="project-item-content">
                <div>
                  {{ item.pipeline == "SAAS" ? "Saas" : "Consulting & Grants" }}
                </div>
                <div>
                ...
                >
                  {{ item.stageLabel.split("(")[0].trim() || "N/A" }}
                </div>
              </div>
            </div>
          </template>
    ...
    <VDialog v-model="showCopyConfirm" persistent max-width="500px">
      <VCard>
        <VCardTitle> Confirm Copy Action </VCardTitle>
        <VCardText>
          <div v-if="copyActionInfo">
            <p>
              You are about to create a project in
              {{ copyActionInfo.type }} with the following fields:
            </p>
            <div
              :style="{
                padding: '8px',
            ...
            >
              <span v-for="(val, key) in copyActionInfo.fields" :key="key">
                <span v-if="val"
                  ><strong>{{ key }}:</strong> {{ val }}</span
                >
              </span>
            </div>
            <div
              v-if="copyActionInfo.type === 'Basecamp'"
    ...
    <VSnackbar v-model="showSnackbar" :timeout="3000" :multi-line="true">
      <div>
...
} from "@/services/basecamp/basecamp";
import { createRunnProject } from "@/services/runn/runn";
import { notifyOnSlack } from "@/services/slack/slack";
import {
  RUNN_APP_BASE_URL,
  RUNN_INTERNAL_CLIENT_ID,
  BASECAMP_EXCLUDED_PROJECT_IDS,
  BASECAMP_PROJECT_TEMPLATES,
  getDefaultRunnClient,
  BASECAMP_COPY_SUBSCRIBERS_LABEL,
} from "@/config/appConstants";

const runnAppBase = RUNN_APP_BASE_URL;
const basecampTemplates = BASECAMP_PROJECT_TEMPLATES;
const selectedBasecampTemplate = ref(undefined);
const selectedRunnClient = ref(getDefaultRunnClient());
...
const filteredRunnProjects = computed(() => {
  ...
  return runnProjects.value.filter(
    (proj) =>
      proj.name.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
      (runnClients.value.find((c) => c.id === proj.clientId)?.name || "")
        .toLowerCase()
...
async function copyRunnToBasecamp(runnProject) {
  const basecampProject = {
    ...
    subscribers: BASECAMP_COPY_SUBSCRIBERS_LABEL,
    todoList:
      runnProject.phases && runnProject.phases.length > 0
        ? runnProject.phases.length + " phases"
        : "No phases",
  };
  openCopyConfirm("Basecamp", runnProject, basecampProject, async () => {
    snackbarHeader.value = `Creating Basecamp project from "${runnProject.name}"`;
    const templateKey =
      basecampTemplates.find((t) => t.value === selectedBasecampTemplate.value)
        ?.title || "None";
    snackbarNotifications.value = [
      "Setting up project from template... (Template: " + templateKey + ")",
    ];
    showSnackbar.value = true;
    // Pass template to API if needed, or just log for now
    const createdProject = await createBasecampProject({
      ...basecampProject,
      template: selectedBasecampTemplate.value,
    }).catch((err) => {
      console.error("Error creating Runn project:", err);
      snackbarHeader.value =
        "Error: " +
    ...
    const todoSetId = createdProject.data.dock.find(
      (dockItem) => dockItem.name === "todoset",
    ).id;
    ...
    console.log(
      ...
      todoSetId,
      "Template:",
      selectedBasecampTemplate.value,
    );
    ...
    notifyOnSlack(
      `New Basecamp Project made from template "${templateKey}": ${urlToNewProject}`,
    );
  });
}
...
async function copyHubspotToRunn(hubspotDeal) {
  ...
  openCopyConfirm("Runn", hubspotDeal, runnProject, async () => {
    ...
    snackbarNotifications.value = [];
    showSnackbar.value = true;
    const newRunnProject = await createRunnProject(
      runnProject.name,
      runnProject.budget,
      runnProject.managerIds,
      selectedRunnClient.value.id,
    ).catch((err) => {
      console.error("Error creating Runn project:", err);
      snackbarHeader.value =
        "Error: " +
    ...
    const urlToNewProject = `${runnAppBase}/projects/${newRunnProject.data.id}`;
    notifyOnSlack(
      `New Runn Project for client "${selectedRunnClient.value?.name || "N/A"}": ${urlToNewProject}`,
    );
  });
}
...
const loadRunnProjects = async () => {
  try {
    ...
    runnProjects.value = res.data
      .filter(
        (proj) =>
          !proj.isArchived &&
          (RUNN_INTERNAL_CLIENT_ID == null ||
            proj.clientId !== RUNN_INTERNAL_CLIENT_ID),
      )
      .map((proj) => {
        // Map client names
...
const loadBasecampProjects = async () => {
  try {
    ...
    basecampProjects.value = res.data.filter(
      (proj) => !BASECAMP_EXCLUDED_PROJECT_IDS.includes(proj.id),
    );
