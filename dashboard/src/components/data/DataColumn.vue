    ...
    >
      <VTooltip :location="computedTooltipLocation" v-if="$slots.details">
        <template v-slot:activator="{ props }">
          <VCard v-bind="props" class="item-box">
            <slot :item="item">{{
              item[sortField] || JSON.stringify(item)
            }}</slot>
          </VCard>
        </template>
        <VCard class="item-tooltip">
...
const props = defineProps({
  ...
  tooltipLocation: {
    ...
    validator: (v) =>
      ["left", "right", "top", "bottom", "auto-right-to-left"].includes(v),
  },
  pageLength: {
    type: Number,
    default: 8,
...
const pageCount = computed(() => {
  return Math.max(
    1,
    Math.ceil(filteredAndSortedItems.value.length / props.pageLength),
  );
});
