...
export const useAuthStore = defineStore("auth", {
  state: () => {
    ...
    if (token) {
      try {
        ...
        console.log(
          "[authStore] error decoding token during initialization:",
          e,
        );
      }
    }
  ...
  getters: {
    isLoggedIn: (state) => {
      if (process.env.VUE_APP_IN_DEVELOPMENT === "true") {
        console.log(
          "[authStore] isLoggedIn getter called in development mode, returning true",
        );
  ...
  actions: {
    ...
    async checkAuthStatus() {
      if (process.env.VUE_APP_IN_DEVELOPMENT === "true") {
        console.log(
          "[authStore] checkAuthStatus called in development mode, setting dummy auth state",
        );
        this.isAuthenticated = true;
      ...
      if (token) {
        try {
          ...
          if (!decoded.exp || Date.now() >= decoded.exp * 1000 - 60000) {
            console.log(
              "[authStore] checkAuthStatus: token expired or about to expire, attempting refresh",
            );
            const refreshed = await this.refreshToken();
            console.log(
              "[authStore] checkAuthStatus: refreshToken result:",
              refreshed,
            );
            if (refreshed && refreshed.accessToken) {
              ...
              console.log(
                "[authStore] checkAuthStatus: token refreshed and decoded:",
                decoded,
              );
              ...
              if (redirectPath) {
                console.log(
                  "[authStore] checkAuthStatus: redirecting to",
                  redirectPath,
                );
          ...
          if (decoded.exp && Date.now() < decoded.exp * 1000) {
            ...
            console.log(
              "[authStore] checkAuthStatus: token valid, isAuthenticated set true",
            );
            return true;
          } else {
            console.log(
              "[authStore] checkAuthStatus: token expired or missing exp",
            );
          }
        } catch (e) {
      ...
      this.userProfileImage = null;
      console.log(
        "[authStore] checkAuthStatus: not authenticated, isAuthenticated set false",
      );
      return false;
    },
